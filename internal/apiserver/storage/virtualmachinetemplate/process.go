...
import (
	...
	tplclient "kubevirt.io/virt-template/client-go/template"
...
type ProcessREST struct {
	client    tplclient.Interface
	processor processor
}

func NewProcessREST(client tplclient.Interface) *ProcessREST {
	return &ProcessREST{
		client:    client,
...
func (p *ProcessREST) Connect(ctx context.Context, id string, _ runtime.Object, r rest.Responder) (http.Handler, error) {
	...
	return http.HandlerFunc(func(_ http.ResponseWriter, req *http.Request) {
		...
		if err = mergeParameters(tpl.Spec.Parameters, opts.Parameters); err != nil {
			r.Error(err)
			return
		}
...
func mergeParameters(tplParams []v1alpha1.Parameter, optsParams []subresourcesv1alpha1.Parameter) error {
	for _, param := range optsParams {
		found := false
		for i := range tplParams {
			if tplParams[i].Name == param.Name {
				tplParams[i].Value = param.Value
				found = true
				break
			}
		}
		if !found {
			return fmt.Errorf("parameter %s not found in template", param.Name)
		}
	}
	return nil
}
